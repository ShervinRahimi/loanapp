...
from loans.models import RequestHeader,Business,Owners,CFAData,Address
...
def status(request):
    ...
    r = RequestHeader.objects.get(filter_id=id)
    b = r.business
...
def add_to_model(parsed):
    ...
    if(Business.objects.filter(name=biz_name).exists()):
        ...
        r = RequestHeader(loan_amount=loan_amount, credit_history=credit_history, entity_type=entity_type, filter_id=filter_id, business=b)
        r.save()